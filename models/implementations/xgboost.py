...
class XGBoost(Model):
    def __init__(self, csv_file=None, seed = None):
    ...
    def get_new_duration(self, operation: Operation):
        """
        ...
        inferenced_variables = pd.DataFrame([features])

        # Predict duration
        predicted_duration = self.predict(inferenced_variables)
    ...
    def inference(self, operation: Operation) -> tuple[int, list[tuple]]:
        ...
        return round(self.get_new_duration(operation), 0), None
